...
def add_routes(app):
    ...
    def get_scan_instructions(project=None):
        ...
        model: ScanningModel = jsonizer.load_first(path) or legacy.load_first(path, ScanningModelFactory)

        if model is None:
            compile_instructions = [
                convert_path_to_url("/api/compile/instructions", p)
        ...
        scan_instructions = [
            convert_path_to_url(base_url, c)
            ...
            return jsonify(**json_response(
                ...
                dict(
                    instructions={
                        ...
                        'auxillary_info': {
                            'culture_freshness':
                                model.auxillary_info.culture_freshness
                                if model.auxillary_info.culture_freshness
                                else None,
                            'culture_source':
                                model.auxillary_info.culture_source.name
                                if model.auxillary_info.culture_source
                                else None,
                            'pinning_project_start_delay':
                                model.auxillary_info.pinning_project_start_delay,  # noqa: E501
                            'plate_age': model.auxillary_info.plate_age,
                            'precultures': model.auxillary_info.precultures,
                            'plate_storage':
                                model.auxillary_info.plate_storage.name
                                if model.auxillary_info.plate_storage
                                else None,
    ...
    def get_scanner_modes(model):
        ...
        if not modes:
            jsonify(
                success=False,
                is_endpoint=True,
                reason=f"Scanner model '{model}' unknown",
            )

        mode_to_text = {
            sane.SCAN_MODES.TPU: "Transparency",
