...
d3.scanomatic.plateHeatmap = function () {
  ...
  function update() {
    ...
    function composeMetadata() {
      ...
      for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < cols; j++) {
          ...
          const col = {
            col: j, row: i, celStartX: x, celStartY: y, celEndX: x + heatMapCelWidth, celEndY: y + heatMapCelHeight, phenotype: data[i][j], metaGT: metaGt, metaGtWhen, metaYield: metaYiled, metaType: metaData.type,
          };
          row.push(col);
        }
        plate.push(row);
    ...
    function findPlateMetaData(row, col, metaData) {
      for (let typeI = 0; typeI < metaData.length; typeI++) {
        const type = metaData[typeI];
        for (let itemI = 0; itemI < type.length; itemI++) {
    ...
    function addmetaDataType(metaDataType, typeName) {
      const dataType = [];
      let badDataElement;
      for (let k = 0; k < metaDataType[0].length; k++) {
