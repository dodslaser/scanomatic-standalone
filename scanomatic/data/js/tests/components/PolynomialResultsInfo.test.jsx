...
import './enzyme-setup';
import PolynomialResultsInfo, { PolynomialEquation, ScientificNotation, numberAsScientific } from '../../ccc/components/PolynomialResultsInfo';
...
describe('<PolynomialEquation />', () => {
  ...
  it('renders the correct x-powers', () => {
    ...
    const wrapper = shallow(<PolynomialEquation {...props} coefficients={[1, 0, 1, 1, 1]} />);
  ...
  it('renders y=0 if no non-zero coeffs', () => {
    const wrapper = shallow(<PolynomialEquation {...props} coefficients={[0, 0, 0]} />);
    expect(wrapper.find('p.math').find('span.variable').first().text())
      .toEqual('y');
    expect(wrapper.find('p.math').text())
      .toContain(' = 0');
  ...
  it('renders y=`intercept` if only intercept', () => {
    const wrapper = shallow(<PolynomialEquation {...props} coefficients={[0, 0, 5]} />);
    expect(wrapper.find('p.math').find('span.variable').first().text())
      .toEqual('y');
...
describe('<ScientificNotation />', () => {
  it('renders a span', () => {
    const wrapper = shallow(<ScientificNotation value={0} precision={1} />);
