...
let new_fixture_data_id;
let new_fixture_detect_id;
let new_fixture_image_id;
let new_fixture_markers_id;
let selected_fixture_div_id;
let fixture_name_id;
let selected_fixture_canvas_id;
let new_fixture_name;
let save_fixture_action_id;
let save_fixture_button;
let remove_fixture_id;
let grayscale_type_id;


let context_warning = '';
let fixture_image = null;
let fixture_name = null;
let markers = null;
let scale = 1;
let areas = [];
let creatingArea = null;
let selected_fixture_canvas_jq;
let selected_fixture_canvas;
let grayscale_graph = null;
let img_width = 0;
let img_height = 0;

$(document.documentElement).mouseup((event) => {
  if (creatingArea != null && selected_fixture_canvas_jq != null) { mouseUpFunction(event); }
});
...
function set_canvas() {
  ...
  selected_fixture_canvas_jq.mousedown((event) => {
    if (context_warning) {
      context_warning = null;
      return;
    }

    const canvasPos = GetMousePosRelative(event, selected_fixture_canvas_jq);
    ...
    if (event.button === 0) {
      if (nextArea < 0) {
        ...
        if (areas[creatingArea].grayscale) { grayscale_graph = null; }
  ...
  selected_fixture_canvas_jq.mousemove((event) => {
    const canvasPos = GetMousePosRelative(event, selected_fixture_canvas_jq);
    const imagePos = translateToImageCoords(canvasPos);
...
function mouseUpFunction(event) {
  ...
  if (isArea(curArea)) {
    ...
    const imagePos = translateToImageCoords({ x: img_width, y: img_height });
  ...
  for (let i = 0; i < areas.length; i++) {
    if (getAreaSize(i) < minUsableSize) {
      ...
      areas.splice(i, 1);
      if (i < curArea) { curArea--; } else if (i == curArea) { curArea = -1; }
      i--;
    }
  }

  if (curArea >= 0) {
    if (hasGrayScale()) { areas[curArea].plate = 0; } else { testAsGrayScale(curArea); }
  }
  setPlateIndices();
  draw_fixture();
}
...
function getAreaSize(plate) {
  if (isInt(plate)) {
    if (plate >= 0 && plate < areas.length) { plate = areas[plate]; } else { plate = null; }
  }
...
function getAreaCenter(plate) {
  ...
  return {
    x: selected_fixture_canvas.width / 2,
...
function setPlateIndices() {
  areas.sort((a, b) => {
    if (a.grayscale) { return -1; } else if (b.grayscale) { return 1; }

    if (a.y2 < b.y1) { return -1; } else if (b.y2 < a.y1) { return 1; } else if (a.x2 < b.x1) { return 1; } else if (b.x2 < a.x1) { return -1; }
...
function getAreaByPoint(point) {
  for (let len = areas.length, i = 0; i < len; i++) {
...
function testAsGrayScale(plate) {
  if (isInt(plate)) {
    if (isArea(plate)) { plate = areas[plate]; } else { plate = null; }
  }

  if (plate) {
    const grayscale_name = GetSelectedGrayscale();
    $.ajax({
      ...
      success(data) {
        ...
        if (data.grayscale && hasGrayScale() === false) {
          ...
          grayscale_graph = GetLinePlot(
            data.target_values, data.source_values,
            'Grayscale', 'Targets', 'Measured values',
          );
          InputEnabled($(grayscale_type_id), false);
        } else {
          if (!hasGrayScale() && data.reason) { grayscale_graph = GetLinePlot([], [], data.reason, 'Targets', 'Measured values'); }
...
function SetAllowDetect() {
  const disallow = $(new_fixture_image_id).val() === '' || $(new_fixture_name).val() === '';
  InputEnabled($(new_fixture_detect_id), !disallow);
}

function get_fixtures() {
  const options = $(current_fixture_id);
  options.empty();
  $.get('/api/data/fixture/names', (data, status) => {
...
function get_fixture() {
  ...
  $.ajax({
    ...
    success(data) {
      ...
      } else if (data.reason) { context_warning = data.reason; } else { context_warning = 'Unknown error retrieving fixture'; }
      draw_fixture();
    },
  });
}
...
function detect_markers() {
  ...
  const button = $(save_fixture_button);
  ...
  $.ajax({
    ...
    processData: false,
    success(data) {
      const new_image = $(new_fixture_image_id);
  ...
  $.ajax({
    ...
    success(data) {
      if (data.success) {
        ...
        if (data.reason) { context_warning = `Save refused: ${data.reason}`; } else { context_warning = 'Save refused'; }
        InputEnabled(button, true);
      }
      draw_fixture();
    },
          ...
          $.ajax({
            ...
            success(data) {
              if (data.success) {
                $(selected_fixture_div_id).hide();
                Dialogue(
                  'Fixture', `Fixture "${fixture_name}" has been removed`,
                  '(A backup is always stored in the fixture config folder)', '?',
                );
              } else {
                console.log(data);
                if (data.reason) { context_warning = data.reason; } else { context_warning = 'Unknown removal issue'; }
...
function draw_fixture() {
  const canvas = selected_fixture_canvas;
  ...
  if (markers) {
    ...
    for (let len = markers.length, i = 0; i < len; i += 1) {
      draw_marker(
        context, markers[i][0] * scale * marker_scale,
  ...
  if (areas) {
    for (let i = 0; i < areas.length; i += 1) { draw_plate(context, areas[i]); }
  }

  if (grayscale_graph) {
    const graph_width = (canvas.width - img_width);
...
function get_updated_scale(canvas, obj) {
  const x_scale = canvas.width / obj.width;
  const y_scale = canvas.height / obj.height;
